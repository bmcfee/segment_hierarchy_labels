...
motifs to large-scale functional components. When inferring the structure of a piece,
different listeners may attend to different temporal scales, which can result in
disagreements when multiple annotators describe the same piece. By quantifying these
disagreements, previous research has yielded several insights relevant to the study
of music cognition: annotators tend to agree when quantifying the degree of ambiguity
of music segment boundaries~\cite{bruderer2008perception}; this ambiguity seems to
depend on musical features, time scale, and genre~\cite{smith2014audio};
it is possible to tune current evaluation metrics to better align with these
perceptual differences~\cite{nieto2014perceptual}. However, previous work has been unable to directly analyze the effects of hierarchical structure since the existing methods for comparing structural annotations do not readily generalize to hierarchical annotations.

In this manuscript, we extend and generalize previous work on hierarchical segment
boundary evaluation~\cite{mcfee2015_hierarchical_eval}. We derive a general family of evaluation metrics which can compare hierarchical annotations in terms of both boundary detection and segment label agreement. Using these metrics, we investigate inter-annotator agreement on the multi-level annotations of the SPAM and SALAMI corpora, and measure baseline performance of a variety of commonly used segmentation algorithms.
\tiny

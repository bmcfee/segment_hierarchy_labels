...
\def\firstAuthorLast{McFee {et~al.}} %use et al only if is more than 1 author
...
\def\Address{$^{1}$Center for Data Science, New York University, New York, NY, USA\\
$^{2}$Pandora, Inc., Oakland, CA, USA\\
$^{3}$Music and Audio Research Laboratory, Department of Music and Performing Arts Professions, New York University, New York, NY, USA}
% The Corresponding Author should be marked with an asterisk
% Provide the exact contact address (this time including street name and city zip code) and email of the corresponding author
\def\corrAuthor{Brian McFee}
\def\corrAddress{Center for Data Science, New York University\\
726 Broadway, 7th floor\\
New York, NY, 10003, USA}
\def\corrEmail{brian.mcfee@nyu.edu}
...
\section{}
Music tends to exhibit structure at multiple scales, ranging from measures and small motifs to large-scale functional components. When inferring the structure of a piece, different listeners may attend to different temporal scales, which can result in disagreements when multiple annotators describe the same piece. By quantifying these disagreements, previous research has yielded several insights relevant to the study of music cognition: annotators tend to agree when quantifying the degree of ambiguity of music segment boundaries [Bruderer, 2008]; this ambiguity seems to depend on musical features, time scale, and genre [Smith, Ching-Hua, and Chew, 2014]; it is possible to tune current evaluation metrics to better align with these perceptual differences [Nieto, Farbood, Jehan, and Bello, 2014]. However, previous work has been unable to directly analyze the effects of hierarchical structure since the existing methods for comparing structural annotations do not readily generalize to hierarchical annotations.

In this manuscript, we extend and generalize previous work on hierarchical segment boundary evaluation [McFee, Nieto, and Bello, 2015]. We derive a general family of evaluation metrics which can compare hierarchical annotations in terms of both boundary detection and segment label agreement. Using these metrics, we investigate inter-annotator agreement on the multi-level annotations of the SPAM and SALAMI corpora, and measure baseline performance of a variety of commonly used segmentation algorithms.
\tiny
 \keyFont{ \section{Keywords:} structure, hierarchy, evaluation} %All article types: you may provide up to 8 keywords; at least 5 are mandatory.
\end{abstract}
